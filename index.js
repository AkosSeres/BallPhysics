...
`;var me=class extends HTMLElement{constructor(){super();this.attachShadow({mode:"open"}),this.shadowRoot.appendChild(ue.content.cloneNode(!0)),this.shadowRoot.appendChild(f("div",null,f("label",{htmlFor:"colorWell",className:"picker-label"},f("div",null,f("slot",null)),f("input",{type:"color",id:"colorWell"}))))}get picker(){return this.shadowRoot.querySelector("#colorWell")}set value(t){this.picker.value=t,this.picker.style["background-color"]=t}set onChange(t){let e=n=>{t(n.target.value),this.picker.style["background-color"]=n.target.value};this.picker.onchange=e,this.picker.oninput=e}};window.customElements.define("color-picker",me);var Mt=35,Ft=.5,Dt=1.5,zt=ie,fe=document.createElement("div"),hs={name:"Ball creator",description:"",element:fe,drawFunc(s,t){let e=s.cnv.getContext("2d");e.strokeStyle="black",e.beginPath(),e.arc(s.mouseX,s.mouseY,Mt,0,2*Math.PI),e.stroke(),s.lastX!==0&&s.lastY!==0&&(e.beginPath(),e.moveTo(s.mouseX,s.mouseY),e.lineTo(s.lastX,s.lastY),e.stroke())},startInteractionFunc(s){},endInteractionFunc(s){if(s.lastX!==0&&s.lastY!==0){let t=new k(X.Circle(Mt,new i(s.lastX,s.lastY)),1,Ft,Dt);t.vel=new i(s.lastX-s.mouseX,s.lastY-s.mouseY),t.style=zt,Number.isFinite(t.pos.x)&&Number.isFinite(t.pos.y)&&Number.isFinite(t.vel.x)&&Number.isFinite(t.vel.y)||(t.vel.x=0,t.vel.y=0),s.physics.addBody(t)}}};fe.append(f("range-slider",{min:5,max:120,step:1,value:Mt,onChange:s=>{Mt=s}},"Size"),f("range-slider",{min:0,max:1,step:.02,value:Ft,onChange:s=>{Ft=s}},"Bounciness"),f("range-slider",{min:0,max:2,step:.1,value:Dt,onChange:s=>{Dt=s}},"Coefficient of friction"),f("color-picker",{value:zt,onChange:s=>{zt=s}},"Color:"));var pe=hs;var ds=document.createElement("div"),us={name:"Delete",description:"",element:ds,drawFunc(s,t){},startInteractionFunc(s){s.choosed&&typeof s.choosed!="boolean"&&s.choosed instanceof k&&s.physics.removeObjFromSystem(s.choosed)}},be=us;var ge=0,ye=0,ms=document.createElement("div"),fs={name:"Move",description:"",element:ms,drawFunc(s,t){let{choosed:e}=s;e instanceof k&&e.m!==0&&(e.move(new i(s.mouseX-e.pos.x,s.mouseY-e.pos.y)),t===0?(e.vel.x=0,e.vel.y=0):(e.vel.x=(s.mouseX-ge)/t,e.vel.y=(s.mouseY-ye)/t),e.ang=0),ge=s.mouseX,ye=s.mouseY},startInteractionFunc(s){let{choosed:t}=s;if(t instanceof k&&t.m!==0){let e=s;e.cnv.style.cursor="grabbing"}},endInteractionFunc(s){let{choosed:t}=s;if(t instanceof k&&t.m!==0){let e=s;e.cnv.style.cursor="grab"}},activated(s){let t=s;t.cnv.style.cursor="grab"},deactivated(s){let t=s;t.cnv.style.cursor="default"}},xe=fs;var ps=document.createElement("div"),bs={name:"Rectangle wall",description:"",element:ps,drawFunc(s,t){if(s.lastX!==0&&s.lastY!==0){let e=s.cnv.getContext("2d");e.strokeStyle="black",e.strokeRect(s.mouseX,s.mouseY,s.lastX-s.mouseX,s.lastY-s.mouseY)}},startInteractionFunc(s){},endInteractionFunc(s){if(s.lastX!==0&&s.lastY!==0){if(Math.abs(s.lastX-s.mouseX)<5&&Math.abs(s.lastY-s.mouseY)<5)return;s.physics.addRectWall(s.lastX/2+s.mouseX/2,s.lastY/2+s.mouseY/2,2*Math.abs(s.lastX/2-s.mouseX/2),2*Math.abs(s.lastY/2-s.mouseY/2));let t=s;t.physics.bodies[t.physics.bodies.length-1].style=S.Beige}}},ve=bs;var Nt=.2,jt=.5,Ht=Q,we=document.createElement("div"),gs={name:"Rectangle body",description:"",element:we,drawFunc(s,t){let e=s.cnv.getContext("2d");s.lastX!==0&&s.lastY!==0&&(e.strokeStyle="black",e.strokeRect(s.mouseX,s.mouseY,s.lastX-s.mouseX,s.lastY-s.mouseY))},startInteractionFunc(s){},endInteractionFunc(s){if(s.lastX!==0&&s.lastY!==0){let t=Math.abs(s.mouseX-s.lastX),e=Math.abs(s.mouseY-s.lastY);if(t/e>50||e/t>50||e===0||t===0)return;s.physics.addRectBody(s.lastX/2+s.mouseX/2,s.lastY/2+s.mouseY/2,2*Math.abs(s.lastX/2-s.mouseX/2),2*Math.abs(s.lastY/2-s.mouseY/2),jt,Nt,Ht)}},keyGotUpFunc(s){},keyGotDownFunc(s){}};we.append(f("range-slider",{min:0,max:.6,step:.02,value:Nt,onChange:s=>{Nt=s}},"Bounciness"),f("range-slider",{min:0,max:2,step:.1,value:jt,onChange:s=>{jt=s}},"Coefficient of friction"),f("color-picker",{value:Ht,onChange:s=>{Ht=s}},"Color:"));var ke=gs;var St=35,qt=.5,Vt=.5,Pt=4,It=24,Wt=Q,Me=document.createElement("div");function Se(s=24,t=4){return[...new Array(s).keys()].map(e=>i.fromAnglePNorm(Math.PI*2*e/s,t))}var ys={name:"Squircle creator",description:"",element:Me,drawFunc(s,t){let e=s.cnv.getContext("2d");e.strokeStyle="black";let n=Se(It,Pt);n.forEach(o=>o.mult(St)),e.beginPath(),e.moveTo(s.mouseX+n[0].x,s.mouseY+n[0].y);for(let o=1;o<n.length;o+=1)e.lineTo(s.mouseX+n[o].x,s.mouseY+n[o].y);e.closePath(),e.stroke(),s.mouseDown&&(e.beginPath(),e.moveTo(s.mouseX,s.mouseY),e.lineTo(s.lastX,s.lastY),e.stroke())},startInteractionFunc(s){},endInteractionFunc(s){let t=Se(It,Pt),e=new i(s.lastX,s.lastY);if(t.forEach(n=>{n.mult(St),n.add(e)}),s.lastX!==0&&s.lastY!==0){let n=new k(X.Polygon(t),1,qt,Vt);n.vel=new i(s.lastX-s.mouseX,s.lastY-s.mouseY),n.style=Wt,s.physics.addBody(n)}}};Me.append(f("range-slider",{min:5,max:120,step:1,value:St,onChange:s=>{St=s}},"Size"),f("range-slider",{min:2,max:7,step:1,value:9-Pt,onChange:s=>{Pt=9-s}},"Roundness"),f("range-slider",{min:12,max:36,step:1,value:It,onChange:s=>{It=s}},"Resolution"),f("range-slider",{min:0,max:.9,step:.02,value:qt,onChange:s=>{qt=s}},"Bounciness"),f("range-slider",{min:0,max:2,step:.1,value:Vt,onChange:s=>{Vt=s}},"Coefficient of friction"),f("color-picker",{value:Wt,onChange:s=>{Wt=s}},"Color:"));var Pe=ys;var j=35;var Gt=1.5,Ut=24,Jt=1,Ie=document.createElement("div"),xs={name:"Soft square creator",description:"",element:Ie,drawFunc(s,t){let e=s.cnv.getContext("2d");e.strokeStyle="black",e.beginPath(),e.moveTo(s.mouseX-j,s.mouseY-j),e.lineTo(s.mouseX+j,s.mouseY-j),e.lineTo(s.mouseX+j,s.mouseY+j),e.lineTo(s.mouseX-j,s.mouseY+j),e.lineTo(s.mouseX-j,s.mouseY-j),e.stroke(),s.lastX!==0&&s.lastY!==0&&(e.beginPath(),e.moveTo(s.mouseX,s.mouseY),e.lineTo(s.lastX,s.lastY),e.stroke())},startInteractionFunc(s){},endInteractionFunc(s){s.lastX!==0&&s.lastY!==0&&s.physics.addSoftSquare(new i(s.lastX,s.lastY),j*2,Gt,new i(s.lastX-s.mouseX,s.lastY-s.mouseY),Ut,Jt)}};Ie.append(f("range-slider",{min:5,max:100,step:1,value:j,onChange:s=>{j=s}},"Size"),f("range-slider",{min:.4,max:3,step:.1,value:Jt,onChange:s=>{Jt=s}},"Pressure"),f("range-slider",{min:0,max:2,step:.1,value:Gt,onChange:s=>{Gt=s}},"Coefficient of friction"),f("range-slider",{min:16,max:48,step:8,value:Ut,onChange:s=>{Ut=s}},"Resolution"));var Ce=xs;var Ee=document.createElement("template");Ee.innerHTML=`
  <style>
    .checkbox-label {
        font-weight: bold;
        padding-right: 0.45em;
