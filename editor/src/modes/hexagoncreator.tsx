 ...
 */
function generateHexagon(offset: Vec2) {
  let os = offset;
  if (offset === undefined) os = new Vec2(0, 0);
  return Shape.Polygon([...new Array(6).keys()].map((n) => {
...
const HexagonCreatorMode: Mode = {
  ...
  drawFunc(editorApp: EditorInterface, _dt: number) {
    ...
    if (editorApp.mouseDown) {
      ...
      ctx.beginPath();
      generateHexagon(mouse).points.forEach((p, i) => {
...
export default HexagonCreatorMode;
