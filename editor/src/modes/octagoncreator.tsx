 ...
 */
function generateOctagon(offset: Vec2) {
  let os = offset;
  if (offset === undefined) os = new Vec2(0, 0);
  return Shape.Polygon([...new Array(8).keys()].map((n) => {
...
const OctagonCreatorMode: Mode = {
  ...
  drawFunc(editorApp: EditorInterface, _dt: number) {
    ...
    if (editorApp.mouseDown) {
      ...
      ctx.beginPath();
      generateOctagon(mouse).points.forEach((p, i) => {
...
export default OctagonCreatorMode;
